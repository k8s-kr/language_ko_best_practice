...
#### [이슈] 한글화 용어집 미등록 용어 처리

#### 설명
- 쿠버네티스 한글화 팀은 국립국어원에서 정한 표준어 사용을 우선으로 한다.
- 단, 논의를 통해 표준어가 아니더라도 업계에서 널리 사용되는 단어는 용어집에 등록할 수 있다.
- 표준어로 번역할 경우 오히려 어색하게 느껴지는 단어가 이에 해당한다.
...
- 따라서, **리뷰 도중에는 커밋을 누적**하여 변경 의도를 투명하게 보여 주고, **최종 승인 직전에만 squash** 하는 것이 바람직하다.
- 단, 이는 리뷰어와 기여자 간의 합의에 따라 조정할 수 있는 원칙이며, 프로젝트의 컨벤션 또는 상황에 따라 예외가 존재할 수 있다.
...
- 결과적으로 영문 원문과 번역문의 **총 라인 수는 동일**해야 한다.
...
- 원문 구조를 과도하게 따르는 직역투 표현을 피하고, 한국어 어순에 맞게 조정한다.
